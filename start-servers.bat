@echo off
REM CitiTrack - Start All Servers
...
start "CitiTrack Backend Server" cmd /k "cd /d %~dp0server && set PORT=5003 && npm install && node server.js"
...
start "CitiTrack Frontend Server" cmd /k "cd /d %~dp0client && npm install && npm start"
